...
%	\begin{enumerate}
%		\item Step 1: Both sides are isomorphic abstractly, as $\mathbb{Z}$ copies of $\Perf(C_{\varphi})$. Here for the $Bun_G$ side, we can argue using the spectral action.
%	    \item Step 2: By compatibility with central character, we are reduced to the degree $0$ part.
%	    \item Step 3: The degree $0$ part follows from compatibility of Spectral action with the maps between Bernstein centers and Helm-Moss (we can even avoid using Helm Moss).
%	\end{enumerate}
%
...
We can get a similar description of $\mathcal{D}^{C_{\varphi}}(G_b(F), \overline{\mathbb{Z}}_{\ell})$ (with arbitrary $b$) for free by the spectral action and its compatibility with $\pi_1(G)_{\Gamma}$-grading. For this, we consider the composition
...
%is an equivalence of categories. For this, we can refer to \cite{helm2018converse} (\textcolor{red}{?}). 
%
%\subsection{The full equivalence}
%Now we can use the compatibility of the spectral action with the $\pi_1(G)_{\Gamma}$-grading to get the full equivalence \ref{Equiv}. For this, we refer to \cite{zou2022categorical}.
...
We now show that $\Theta$ induces a derived equivalence on the degree zero part. Before that, we do some preparations.
...
%	\item The restriction of the Whittaker representation $W_{\vartheta, [\pi]}$ is a (finitely generated) projective generator of $\Rep_{\Lambda}(G(F))_{[\pi]}$. For projectivity, see \cite[Section 3]{helm2016whittaker}. For being a generator, I cann't find a reference for $\overline{\mathbb{Z}}_{\ell}$-coefficients. But I believe the same argument as the $\overline{\mathbb{Q}}_{\ell}$-coefficients (see \cite[Section 39]{bushnell2006local}; note their definition of Whittaker representation is dual to our definition, as an induction instead of compact induction. See also, \cite[Section 2.1 and others]{bushnell2003generalized}) works. (\textcolor{red}{prove if have time})
...
%\textcolor{red}{Do you know that the spectral action preserves (finite) direct sums?} I guess yes, it finally comes from the Hecke action.
...
%	To see it is a generator, it is well known that every supercuspidal representation occurs as a subrepresentation of $W_{\vartheta}$ (see for example \cite[Chapter 36]{bushnell2006local}), by taking the smooth dual, we see they also occur as a quotient of $W_{\vartheta}$ (\textcolor{red}{Check carefully!}), we win!
...
%    Indeed, for any $V \in \Rep_{\overline{\mathbb{Z}}_{\ell}}(G(F))_{[\pi]}$, we can construct a surjective morphism of $\Lambda$-modules
